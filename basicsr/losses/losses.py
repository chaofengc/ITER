...
class LPIPSLoss(nn.Module):
    ...
    """
    def __init__(self, loss_weight = 1.0, spatial=False, retPerLayer=False, reduction='mean'):
        super(LPIPSLoss, self).__init__()
        self.model = pyiqa.create_metric('lpips-vgg', as_loss=True, spatial=spatial, loss_reduction=reduction)
        self.loss_weight = loss_weight
        self.retPerLayer = retPerLayer

    def forward(self, x, gt):
        if self.retPerLayer:
            score = self.model(x, gt, retPerLayer=self.retPerLayer)
            score = score[1]
        else:
            score = self.model(x, gt, retPerLayer=self.retPerLayer)
        if isinstance(score, list):
            return [x * self.loss_weight for x in score], None
        else:
