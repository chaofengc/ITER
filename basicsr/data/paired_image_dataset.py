...
class PairedImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        max_num = opt.get('max_num', 999999999)
                
        self.lq_paths = make_dataset(self.lq_folder)[:max_num]
        self.gt_paths = make_dataset(self.gt_folder)[:max_num]

        if opt['phase'] == 'train':
            filter_key = opt.get('filter_key', 'full')
            self.lq_paths = [x for x in self.lq_paths if filter_key not in x]
            self.gt_paths = [x for x in self.gt_paths if filter_key not in x]

            # self.lq_paths = [x for x in self.lq_paths if 'DIV' not in x and 'OST_train_LR_sub' not in x]
            # self.gt_paths = [x for x in self.gt_paths if 'DIV' not in x and 'OST_train_HR_sub' not in x]

            # self.lq_paths = [x for x in self.lq_paths if 'OST_train_LR_full' in x]
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            assert scale == input_gt_size // input_lq_size, f'Error: {gt_path}, {input_gt_size}, {input_lq_size}'
            ...
            # flip, rotation
            img_gt, img_lq = augment([img_gt, img_lq], self.opt.get('use_flip', False),
